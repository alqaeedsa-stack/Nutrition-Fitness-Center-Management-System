...
export default function VendorBilling(){
  ...
  const filteredBills=bills.filter(x=>{
    const q=billSearch.trim().toLowerCase();
    const matchesSearch=!q || (x.billNumber+' '+(x.vendorInvoiceNumber||'')+' '+x.vendorName+' '+(x.poNumber||'')).toLowerCase().includes(q);
    const matchesStatus=billStatusFilter==='all' || x.status===billStatusFilter;
    return matchesSearch && matchesStatus;
  });
  const filteredPayments=payments.filter(x=>{
    const q=paymentSearch.trim().toLowerCase();
    return !q || (x.paymentNumber+' '+x.vendorName+' '+(x.billNumber||'')+' '+(x.reference||'')).toLowerCase().includes(q);
  });
  ...
  return <main className="app-shell"><div className="odoo-workspace"><PurchaseWorkspaceNav /><div className="odoo-workspace-main">
    ...
    {tab==='payments'&&<section className="staff-management-grid">
      ...
      <section className="panel"><div className="panel-heading-row"><div><p className="eyebrow">سجل المدفوعات</p><h2>سجل المدفوعات</h2></div><button className="secondary-button" onClick={()=>void load()}>تحديث</button></div>
        <label>بحث المدفوعات<input value={paymentSearch} onChange={e=>setPaymentSearch(e.target.value)} placeholder="رقم الدفعة أو المورد أو الفاتورة أو المرجع"/></label><div className="staff-table-wrap"><table className="staff-table"><thead><tr><th>الدفعة</th><th>التاريخ</th><th>المورد</th><th>الفاتورة</th><th>المبلغ</th><th>الطريقة</th><th>المرجع</th></tr></thead><tbody>{filteredPayments.map(p=><tr key={p.id}><td>{p.paymentNumber}</td><td>{p.paymentDate}</td><td>{p.vendorName}</td><td>{p.billNumber||'—'}</td><td>{Number(p.amount).toFixed(2)} ر.س</td><td>{p.paymentMethod}</td><td>{p.reference||'—'}</td></tr>)}{!payments.length&&<tr><td colSpan={7}>لا توجد مدفوعات.</td></tr>}</tbody></table></div>
      </section>
    </section>}
