  ...
  return <main className="app-shell">
    ...
    {tab==='orders'&&<section className="staff-management-grid">
      ...
      <section className="panel"><div className="panel-heading-row"><div><p className="eyebrow">PURCHASE ORDERS</p><h2>سجل طلبات الشراء</h2></div><select value={orderFilter} onChange={e=>setOrderFilter(e.target.value)}><option value="all">كل الحالات</option>{Object.entries(statusLabel).filter(([k])=>k!=='posted').map(([k,v])=><option key={k} value={k}>{v}</option>)}</select></div>
        <div className="staff-table-wrap"><table className="staff-table"><thead><tr><th>الأمر</th><th>المورد</th><th>التاريخ</th><th>الإجمالي</th><th>الحالة</th><th>إجراء</th></tr></thead><tbody>{filteredOrders.map(o=><tr key={o.id}><td>{o.poNumber}</td><td>{o.vendorName}</td><td>{o.orderDate}</td><td>{Number(o.total).toFixed(2)} ر.س</td><td><span className={'status-badge '+(statusClass[o.status]??'inactive')}>{statusLabel[o.status]??o.status}</span></td><td><button className="secondary-button" onClick={()=>void openOrder(o.id)}>فتح</button>{o.status==='draft'&&<button className="text-link button-link" onClick={()=>void status(o.id,'confirmed')}>تأكيد</button>}{o.status==='confirmed'&&<button className="text-link button-link" onClick={()=>void openOrder(o.id)}>استلام</button>}{o.status!=='received'&&o.status!=='cancelled'&&<button className="text-link button-link" onClick={()=>void status(o.id,'cancelled')}>إلغاء</button>}</td></tr>)}</tbody></table></div>
      </section>
    </section>}

    {selected&&<section className="panel"><div className="panel-heading-row"><div><span className="eyebrow">PURCHASE ORDER</span><h2>{selected.order.poNumber} · {selected.order.vendorName}</h2><p>{statusLabel[selected.order.status]??selected.order.status} · {selected.order.orderDate}{selected.order.expectedDate?' · متوقع '+selected.order.expectedDate:''}</p></div><button className="secondary-button" onClick={()=>setSelected(null)}>إغلاق</button></div>
