...
import { OdooKanban, OdooKanbanCard, OdooStatusbar, OdooViewSwitcher } from './components/OdooERP';
  ...
  return <main className="app-shell">
    ...
    <section className="staff-management-grid">
      ...
      <section className="panel"><div className="panel-heading-row"><div><p className="eyebrow">الخطط</p><h2>الخطط الحالية</h2></div><button className="secondary-button" onClick={()=>void load()}>تحديث</button></div>
        ...
        <div className="module-toolbar"><div className="toolbar-filters"><input value={planSearch} onChange={e=>setPlanSearch(e.target.value)} placeholder="بحث باسم الخطة أو العميل أو الأخصائي"/><select value={statusFilter} onChange={e=>setStatusFilter(e.target.value)}><option value="all">كل الحالات</option>{Object.entries(statusLabel).map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></div><OdooViewSwitcher value={view} onChange={v=>setView(v as 'list'|'kanban')} views={['list','kanban']} /></div>
        {!filteredPlans.length?<p className="empty-state">لا توجد خطط مطابقة.</p>:view==='kanban'?<OdooKanban>{filteredPlans.map(p=><OdooKanbanCard key={p.id} title={p.title} subtitle={p.customerName+' '+p.customerLastName} status={statusLabel[p.status] ?? p.status} onClick={()=>setSelectedId(p.id)}><div>{p.specialistName} · {p.startDate}</div><small>الإصدار {p.version}</small></OdooKanbanCard>)}</OdooKanban>:<div className="staff-table-wrap"><table className="staff-table"><thead><tr><th>الخطة</th><th>العميل</th><th>الأخصائي</th><th>البداية</th><th>الحالة</th><th></th></tr></thead><tbody>{filteredPlans.map(p=><tr key={p.id}><td>{p.title}<small> الإصدار {p.version}</small></td><td>{p.customerName} {p.customerLastName}</td><td>{p.specialistName}</td><td>{p.startDate}</td><td>{statusLabel[p.status] ?? p.status}</td><td><button className="secondary-button" onClick={()=>setSelectedId(p.id)}>فتح</button></td></tr>)}</tbody></table></div>}
      </section>
    </section>
    {selected&&<section className="panel"><div className="panel-heading-row"><div><p className="eyebrow">تفاصيل الخطة</p><h2>{selected.title}</h2><p>{selected.customerName} {selected.customerLastName} · {selected.specialistName}</p></div></div><OdooStatusbar steps={[{key:'draft',label:'مسودة'},{key:'active',label:'نشطة'},{key:'completed',label:'مكتملة'}]} current={selected.status}/>
