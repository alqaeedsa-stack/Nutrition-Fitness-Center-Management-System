...
export default function NutritionManagement({ user }: { user: User }) {
  ...
  async function load() {
    ...
    } catch(e){setError(e instanceof Error?e.message:'تعذر تحميل {en ? 'Nutrition Plans' : 'الخطط الغذائية'}');}
  }
  useEffect(()=>{void load()},[]);
  ...
  async function deleteItem(itemId:string) {
    ...
    try{await apiFetch('/nutrition/'+selectedId+'/items/'+itemId,{method:'DELETE'});setMessage('تم {en ? 'Delete' : 'حذف'} عنصر الخطة');await load();}
    catch(e){setError(e instanceof Error?e.message:'تعذر {en ? 'Delete' : 'حذف'} العنصر');}
  }

  const selected=plans.find(p=>p.id===selectedId);
  return <main className="app-shell">
    <header className="app-header"><div><span className="eyebrow">التغذية</span><h1>{en ? 'Nutrition Plans' : 'الخطط الغذائية'}</h1></div><Link className="secondary-button" to="/admin/dashboard">لوحة الإدارة</Link></header>
    ...
    <section className="staff-management-grid">
      <section className="panel"><p className="eyebrow">خطة جديدة</p><h2>إنشاء خطة</h2><form className="form-stack" onSubmit={createPlan}>
        ...
        {canWrite&&<button className="primary-action button" disabled={saving}>{saving?'{en ? 'Saving...' : 'جارٍ الحفظ...'}':'إنشاء الخطة'}</button>}
      </form></section>
