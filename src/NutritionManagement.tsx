...
export default function NutritionManagement({ user }: { user: User }) {
  ...
  return <main className="app-shell">
    <header className="app-header"><div><span className="eyebrow">NUTRITION</span><h1>الخطط الغذائية</h1></div><Link className="secondary-button" to="/admin/dashboard">لوحة الإدارة</Link></header>
    ...
    <section className="staff-management-grid">
      <section className="panel"><p className="eyebrow">NEW PLAN</p><h2>إنشاء خطة</h2><form className="form-stack" onSubmit={createPlan}>
      ...
      <section className="panel"><div className="panel-heading-row"><div><p className="eyebrow">PLANS</p><h2>الخطط الحالية</h2></div><button className="secondary-button" onClick={()=>void load()}>تحديث</button></div>
    ...
    {selected&&<section className="panel"><div className="panel-heading-row"><div><p className="eyebrow">PLAN DETAILS</p><h2>{selected.title}</h2><p>{selected.customerName} {selected.customerLastName} · {selected.specialistName}</p></div></div>
