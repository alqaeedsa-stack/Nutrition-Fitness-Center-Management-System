...
staffRoutes.get('/reports/summary', async c => {
  ...
  const [summaryRows, dailyRows, inventoryRows] = await Promise.all([
    withDatabase(c.env, db => db.select({
      ...
      taxTotal: sql`coalesce(sum(case when ${sales.status = 'completed' then ${sales.tax else 0 end), 0)`,
    }).from(sales).where(and(eq(sales.centerId, auth.user.centerId!), sql`${sales.createdAt >= ${start and ${sales.createdAt < ${end`))),
    withDatabase(c.env, db => db.select({
      date: sql`to_char(date_trunc('day', ${sales.createdAt), 'YYYY-MM-DD')`,
      count: sql`count(*)`,
      total: sql`coalesce(sum(${sales.total), 0)`,
    }).from(sales).where(and(eq(sales.centerId, auth.user.centerId!), eq(sales.status, 'completed'), sql`${sales.createdAt >= ${start and ${sales.createdAt < ${end`))
  ...
  return c.json({
    from, to,
    summary: { salesCount: Number(summary.salesCount), salesTotal: String(summary.salesTotal ?? '0'), taxTotal: String(summary.taxTotal ?? '0'), inventoryValue: inventoryValue.toFixed(2), lowStockCount: lowStock.length },
