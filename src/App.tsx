...
function StaffPOS({ user }: { user: AuthUser }) {
  ...
  const [customer,setCustomer]=useState<Customer|null>(null); const [paymentMethod,setPaymentMethod]=useState<'cash'|'mada'|'card'|'bank_transfer'|'apple_pay'>('cash');
  ...
  return <main className="app-shell"><header className="app-header"><div><span className="eyebrow">نقطة البيع</span><h1>نقطة البيع</h1></div><div className="portal-choice-actions"><button className="secondary-button" type="button" onClick={()=>void loadSalesHistory()}>المبيعات السابقة</button><Link className="secondary-button" to="/admin/dashboard">لوحة الإدارة</Link></div></header>
    ...
    <section className="staff-management-grid"><section className="panel"><p className="eyebrow">بحث المنتجات</p><h2>إضافة المنتجات</h2>
      ...
      {!!products.length&&<div className="cart-list">{products.map(p=><button type="button" className="cart-row" key={p.id} onClick={()=>addProduct(p)}><span><strong>{p.name}</strong><small>{p.sku}{p.barcode?' · '+p.barcode:''} · {['subscription','service'].includes(p.productType ?? 'product')?(p.productType==='subscription'?'اشتراك غير مخزني':'خدمة غير مخزنية'):'المتاح '+p.quantity}</small></span><strong>{p.sellingPrice} ر.س</strong></button>)}</div>}
          ...
          <div className="panel">
            ...
            <div className="form-stack">
              ...
              <div className="form-row">
                ...
                <label><input type="checkbox" checked={Boolean(settingsForm.subscriptionDailyProration)} onChange={e=>updateSetting('subscriptionDailyProration',e.target.checked)}/> احتساب نسبي حسب الأيام</label>
              </div>
            </div>
          </div>
          <div className="form-row">
            <label>حساب الإيراد المؤجل<select value={settingsForm.deferredRevenueAccountId} onChange={e=>updateSetting('deferredRevenueAccountId',e.target.value)}><option value="">افتراضي</option>{accounts.filter(a=>a.accountType==='liability').map(a=><option key={a.id} value={a.id}>{a.code} — {a.name}</option>)}</select></label>
            <label>حساب إيراد الاشتراك<select value={settingsForm.subscriptionRevenueAccountId} onChange={e=>updateSetting('subscriptionRevenueAccountId',e.target.value)}><option value="">افتراضي</option>{accounts.filter(a=>a.accountType==='revenue').map(a=><option key={a.id} value={a.id}>{a.code} — {a.name}</option>)}</select></label>
          </div>
        </>
      )}
...
function AppContent() {
  ...
  return (
    <>
      ...
      <Route path="/admin/pos" element={<ERPRouteChrome title="نقطة البيع">{permissionGuard('pos.read') ? <StaffPOS user={user!} /> : staffGuard ? <Navigate to="/admin/dashboard" replace /> : user ? <Navigate to="/customer/home" replace /> : <Navigate to="/admin" replace />}</ERPRouteChrome>} />
      <Route path="/admin/appointments" element={<ERPRouteChrome title="المواعيد">{permissionGuard('appointments.read') ? <Appointments user={user!} /> : user ? <Navigate to="/admin/dashboard" replace /> : <Navigate to="/admin" replace />}</ERPRouteChrome>} />
      <Route path="/admin/nutrition" element={<ERPRouteChrome title="الخطط الغذائية">{permissionGuard('nutrition.read') ? <NutritionManagement user={user!} /> : user ? <Navigate to="/admin/dashboard" replace /> : <Navigate to="/admin" replace />}</ERPRouteChrome>} />
      <Route path="/admin/fitness" element={<ERPRouteChrome title="الخطط الرياضية">{permissionGuard('fitness.read') ? <FitnessManagement user={user!} /> : user ? <Navigate to="/admin/dashboard" replace /> : <Navigate to="/admin" replace />}</ERPRouteChrome>} />
      <Route path="/admin/customers" element={<ERPRouteChrome title="العملاء">{permissionGuard('customers.read') ? <Customers user={user!} /> : user ? <Navigate to="/admin/dashboard" replace /> : <Navigate to="/admin" replace />}</ERPRouteChrome>} />
      ...
      <Route path="/admin/reports" element={<ERPRouteChrome title="التقارير">{permissionGuard('reports.read') ? <Reports user={user!} /> : user ? <Navigate to="/admin/dashboard" replace /> : <Navigate to="/admin" replace />}</ERPRouteChrome>} />
