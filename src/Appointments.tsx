...
export default function Appointments({ user }: { user: User }) {
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      ...
      setMessage(editingId ? 'تم تحديث الموعد' : 'تم إنشاء الموعد');
      reset(); await load();
    } catch(e) { setError(e instanceof Error ? e.message : 'تعذر حفظ الموعد'); }
    finally { setSaving(false); }
  }
  async function quickStatus(id:string,status:string) {
    try { await apiFetch(`/appointments/${id}`,{method:'PATCH',body:JSON.stringify({status})}); setMessage('تم تحديث حالة الموعد'); await load(); }
  ...
  return <main className="app-shell">
      ...
      <section className="panel">
        ...
        <form className="form-stack" onSubmit={submit}>
          ...
          <div className="form-row">
            <label>بداية الموعد<input type="datetime-local" required value={form.startsAt} onChange={e=>setForm({...form,startsAt:e.target.value})}/></label>
            <label>نهاية الموعد<input type="datetime-local" required value={form.endsAt} onChange={e=>setForm({...form,endsAt:e.target.value})}/></label>
          </div>
          <label>نوع الموعد<input required value={form.appointmentType} onChange={e=>setForm({...form,appointmentType:e.target.value})}/></label>
          <label>الحالة<select value={form.status} onChange={e=>setForm({...form,status:e.target.value})}>{statuses.map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></label>
    ...
    <section className="panel">
      <div className="staff-table-wrap"><table className="staff-table"><thead><tr><th>التاريخ</th><th>العميل</th><th>المختص</th><th>النوع</th><th>الحالة</th><th>إجراء</th></tr></thead>
