...
export const products = pgTable('products', {
  ...
  active: boolean('active').notNull().default(true),
  inventoryValuationMethod: varchar('inventory_valuation_method', { length: 20 }),
  costMethod: varchar('cost_method', { length: 20 }),
  inventoryTracking: varchar('inventory_tracking', { length: 20 }).notNull().default('none'),
  serialAutoGenerate: boolean('serial_auto_generate').notNull().default(true),
  serialTracking: boolean('serial_tracking').notNull().default(false),
  serialPrefix: varchar('serial_prefix', { length: 30 }),
  allowNegativeStock: boolean('allow_negative_stock').notNull().default(false),
  expiryTracking: boolean('expiry_tracking').notNull().default(false),
  posAvailable: boolean('pos_available').notNull().default(true),
  ecommerceAvailable: boolean('ecommerce_available').notNull().default(false),
  ...
  purchaseAllowed: boolean('purchase_allowed').notNull().default(true),
  salesUom: varchar('sales_uom', { length: 30 }).notNull().default('unit'),
  purchaseUom: varchar('purchase_uom', { length: 30 }).notNull().default('unit'),
  minimumSalesPrice: numeric('minimum_sales_price', { precision: 14, scale: 2 }),
  inventoryAccountId: uuid('inventory_account_id'),
  costOfSalesAccountId: uuid('cost_of_sales_account_id'),
  revenueAccountId: uuid('revenue_account_id'),
  purchaseAccountId: uuid('purchase_account_id'),
  salesReturnAccountId: uuid('sales_return_account_id'),
...
export const sales = pgTable('sales', {
  ...
  discount: numeric('discount_total', { precision: 14, scale: 2 }).notNull().default('0'),
...
export const customerSubscriptions = pgTable('customer_subscriptions', {
  ...
  deferredRevenueAccountId: uuid('deferred_revenue_account_id'),
  revenueAccountId: uuid('revenue_account_id'),
  recognitionMethod: varchar('recognition_method', { length: 30 }).notNull().default('monthly'),
  recognizedAmount: numeric('recognized_amount', { precision: 14, scale: 2 }).notNull().default('0'),
  createdBy: uuid('created_by').notNull().references(() => users.id),
  updatedBy: uuid('updated_by').notNull().references(() => users.id),
  ...auditTimestamps,
}, (table) => [
...
export const purchaseReturns = pgTable('purchase_returns', {
  ...
  uniqueIndex('purchase_returns_center_number_uq').on(table.centerId, table.returnNumber),
  index('purchase_returns_vendor_date_idx').on(table.vendorId, table.returnDate),
]);
...
export const purchaseReturnItems = pgTable('purchase_return_items', {
  ...
  index('purchase_return_items_product_idx').on(table.productId),
]);
