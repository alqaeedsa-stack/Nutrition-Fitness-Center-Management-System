...
accountingRoutes.get('/reports/equity-statement',async c=>{
  ...
  return c.json({accounts:r.rows,netIncome:Number((pnl.rows[0] as any)?.net_income??0),otherComprehensiveIncome:Number((oci.rows[0] as any)?.other_comprehensive_income??0)});
});
