    ...
    <main className="auth-page">
      <section className="auth-card" aria-labelledby="login-title">
        ...
        <form onSubmit={submit} className="form-stack">
          <label>البريد الإلكتروني أو الجوال<input value={identifier} onChange={(event) => setIdentifier(event.target.value)} autoComplete="username" required /></label>
          ...
          <button className="primary-action button" type="submit" disabled={loading}>{loading ? 'جارٍ التحقق...' : 'تسجيل الدخول'}</button>
        </form>
...
function Dashboard({ user, onLogout }: { user: AuthUser; onLogout: () => void }) {
  ...
  async function logout() {
    ...
    finally { onLogout(); navigate('/login', { replace: true }); setLogoutLoading(false); }
  }

  return (
    <main className="app-shell">
      <header className="app-header">
        <div className="brand-inline"><div className="brand-mark">N</div><div><span className="eyebrow">Nutrition & Fitness Center</span><h1>لوحة إدارة المركز</h1></div></div>
        <button className="secondary-button" onClick={logout} disabled={logoutLoading}>{logoutLoading ? '...' : 'تسجيل الخروج'}</button>
      </header>
      ...
      {customer && <section className="customer-summary"><div><span className="eyebrow">ملف العميل</span><strong>{customer.fullName ?? 'عميل'}</strong></div><div><span className="eyebrow">الجوال</span><span>{customer.phone ?? '—'}</span></div><div><span className="eyebrow">الحالة</span><span className="active-dot">نشط</span></div></section>}
      ...
      <section className="module-grid" aria-label="وحدات النظام">
        ...
        ].map(([title, code, description]) => <article className="module-card" key={code}><span className="module-code">{code}</span><h3>{title}</h3><p>{description}</p><span className="module-status">قيد البناء</span></article>)}
      </section>

      <footer className="app-footer"><span>حساب المركز: {user.centerId}</span><span>{user.email ?? user.phone ?? 'حساب مستخدم'}</span></footer>
    </main>
  );
}
...
function Home() {
  return (
    <main className="landing-page">
      <nav className="landing-nav"><div className="brand-inline"><div className="brand-mark">N</div><div className="brand-name"><strong>Nutrition</strong><span>& Fitness Center</span></div></div><Link className="nav-login" to="/login">تسجيل الدخول</Link></nav>
      ...
      <section className="hero">
        <div className="hero-copy">
          ...
          <div className="hero-actions"><Link className="primary-action" to="/login">دخول النظام <span>←</span></Link><a className="secondary-hero" href="#features">اكتشف المزايا</a></div>
          <div className="trust-row"><span>بياناتك في مكان واحد</span><span>صلاحيات حسب الدور</span><span>واجهة عربية RTL</span></div>
        </div>
        <div className="hero-visual" aria-hidden="true"><div className="dashboard-window"><div className="window-top"><span></span><span></span><span></span></div><div className="window-content"><div className="mini-sidebar"><b>N</b><i></i><i></i><i></i><i></i></div><div className="mini-main"><div className="mini-title"><span></span><b></b></div><div className="mini-cards"><div></div><div></div><div></div></div><div className="mini-chart"><span></span><span></span><span></span><span></span><span></span><span></span></div></div></div><div className="float-card one"><b>+24</b><span>موعد اليوم</span></div><div className="float-card two"><b>96%</b><span>متابعة العملاء</span></div></div>
      </section>
      ...
      <section className="closing"><div><span className="eyebrow">ابدأ من هنا</span><h2>جاهز لإدارة مركزك بشكل أكثر تنظيمًا؟</h2><p>سجّل الدخول للوصول إلى مساحة العمل الخاصة بك.</p></div><Link className="primary-action" to="/login">دخول النظام <span>←</span></Link></section>
...
export default function App() {
  ...
  useEffect(() => {
    apiFetch<{ user: AuthUser }>('/auth/me').then((result) => setUser(result.user)).catch(() => setUser(null)).finally(() => setAuthChecking(false));
  }, []);

  if (authChecking) return <main className="loading-page"><div className="brand-mark">N</div><p>جارٍ تحميل النظام...</p></main>;

  return <Routes><Route path="/" element={<Home />} /><Route path="/health" element={<Health />} /><Route path="/login" element={user ? <Navigate to="/dashboard" replace /> : <Login onLogin={setUser} />} /><Route path="/dashboard" element={user ? <Dashboard user={user} onLogout={() => setUser(null)} /> : <Navigate to="/login" replace />} /><Route path="*" element={<NotFound />} /></Routes>;
}
