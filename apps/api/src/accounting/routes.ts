...
accountingRoutes.get('/accounts/:id',async c=>{
  ...
  try{
    ...
    const refs=await withDatabase(c.env,db=>db.execute(sql`
      select
        (select count(*) from accounting_accounts where parent_id=${accountId} and center_id=${auth.user.centerId!}) as children,
        (select count(*) from journal_entry_lines l join journal_entries j on j.id=l.journal_entry_id where l.account_id=${accountId} and j.center_id=${auth.user.centerId!}) as journal_lines,
        (select count(*) from products where center_id=${auth.user.centerId!} and (inventory_account_id=${accountId} or cost_of_sales_account_id=${accountId} or revenue_account_id=${accountId} or purchase_account_id=${accountId} or sales_return_account_id=${accountId} or purchase_return_account_id=${accountId} or deferred_revenue_account_id=${accountId} or subscription_revenue_account_id=${accountId})) as product_refs,
        (select count(*) from accounting_settings where center_id=${auth.user.centerId!} and (inventory_account_id=${accountId} or input_vat_account_id=${accountId} or accounts_payable_account_id=${accountId} or cash_bank_account_id=${accountId} or accounts_receivable_account_id=${accountId} or revenue_account_id=${accountId} or output_vat_account_id=${accountId} or cost_of_sales_account_id=${accountId} or deferred_revenue_account_id=${accountId} or subscription_revenue_account_id=${accountId})) as settings_refs,
        (select count(*) from customer_subscriptions where center_id=${auth.user.centerId!} and (deferred_revenue_account_id=${accountId} or revenue_account_id=${accountId})) as subscription_refs
    `));
    return c.json({account:r.rows[0],references:refs.rows[0]});
  }catch(e){return c.json({error:{code:'ACCOUNT_DETAILS_FAILED',message:'تعذر تحميل تفاصيل الحساب',detail:e instanceof Error?e.message:'unknown'}},400);}
});
...
accountingRoutes.get('/reports/other-comprehensive-income',async c=>{
  const auth=await access(c,'accounting.read'); if('error' in auth)return auth.error;
  const dates=reportDates(c); if(!dates)return c.json({error:{code:'INVALID_DATE_RANGE',message:'نطاق التاريخ غير صحيح'}},400);
  const r=await withDatabase(c.env,db=>db.execute(sql`select a.id,a.code,a.name,a.account_type as "accountType",coalesce(sum(case when j.id is not null then l.debit else 0 end),0)::numeric(18,2) as debit,coalesce(sum(case when j.id is not null then l.credit else 0 end),0)::numeric(18,2) as credit from accounting_accounts a left join journal_entry_lines l on l.account_id=a.id left join journal_entries j on j.id=l.journal_entry_id and j.center_id=a.center_id and j.status='posted' and j.entry_date >= ${dates.from} and j.entry_date <= ${dates.to} where a.center_id=${auth.user.centerId!} and a.statement_section='other_comprehensive_income' group by a.id order by a.code`));
  return c.json({accounts:r.rows});
});
accountingRoutes.get('/reports/equity-statement',async c=>{
  const auth=await access(c,'accounting.read'); if('error' in auth)return auth.error;
  const dates=reportDates(c); if(!dates)return c.json({error:{code:'INVALID_DATE_RANGE',message:'نطاق التاريخ غير صحيح'}},400);
  const r=await withDatabase(c.env,db=>db.execute(sql`select a.id,a.code,a.name,a.account_type as "accountType",coalesce(sum(case when j.id is not null and j.entry_date < ${dates.from} then l.debit else 0 end),0)::numeric(18,2) as opening_debit,coalesce(sum(case when j.id is not null and j.entry_date < ${dates.from} then l.credit else 0 end),0)::numeric(18,2) as opening_credit,coalesce(sum(case when j.id is not null and j.entry_date >= ${dates.from} and j.entry_date <= ${dates.to} then l.debit else 0 end),0)::numeric(18,2) as debit,coalesce(sum(case when j.id is not null and j.entry_date >= ${dates.from} and j.entry_date <= ${dates.to} then l.credit else 0 end),0)::numeric(18,2) as credit from accounting_accounts a left join journal_entry_lines l on l.account_id=a.id left join journal_entries j on j.id=l.journal_entry_id and j.center_id=a.center_id and j.status='posted' where a.center_id=${auth.user.centerId!} and a.account_type='equity' group by a.id order by a.code`));
  ...
  const oci=await withDatabase(c.env,db=>db.execute(sql`select coalesce(sum(case when a.account_type in ('revenue','liability','equity') then l.credit-l.debit else l.debit-l.credit end),0)::numeric(18,2) as other_comprehensive_income from accounting_accounts a join journal_entry_lines l on l.account_id=a.id join journal_entries j on j.id=l.journal_entry_id and j.center_id=a.center_id and j.status='posted' where a.center_id=${auth.user.centerId!} and a.statement_section='other_comprehensive_income' and j.entry_date >= ${dates.from} and j.entry_date <= ${dates.to}`));
  const netIncome=Number((pnl.rows[0] as any)?.net_income??0);
  const otherComprehensiveIncome=Number((oci.rows[0] as any)?.other_comprehensive_income??0);
  const accounts=(r.rows as any[]).map(x=>({...x,debit:Number(x.debit),credit:Number(x.credit),openingBalance:Number(x.opening_credit)-Number(x.opening_debit)}));
  if(netIncome!==0) accounts.push({id:'net-income',code:'—',name:'صافي ربح / (خسارة) الفترة',accountType:'equity',debit:netIncome<0?Math.abs(netIncome):0,credit:netIncome>0?netIncome:0,openingBalance:0});
  if(otherComprehensiveIncome!==0) accounts.push({id:'oci-total',code:'—',name:'الدخل الشامل الآخر للفترة',accountType:'equity',debit:otherComprehensiveIncome<0?Math.abs(otherComprehensiveIncome):0,credit:otherComprehensiveIncome>0?otherComprehensiveIncome:0,openingBalance:0});
  return c.json({accounts,netIncome,otherComprehensiveIncome});
});
