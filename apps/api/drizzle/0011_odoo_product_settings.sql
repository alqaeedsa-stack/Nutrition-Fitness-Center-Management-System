-- Odoo-style per-product settings. Center valuation remains the default; product values override it when set.
ALTER TABLE centers
  ADD COLUMN IF NOT EXISTS inventory_valuation_method varchar(20) NOT NULL DEFAULT 'perpetual';

ALTER TABLE products
  ADD COLUMN IF NOT EXISTS inventory_valuation_method varchar(20),
  ADD COLUMN IF NOT EXISTS cost_method varchar(20),
  ADD COLUMN IF NOT EXISTS inventory_tracking varchar(20) NOT NULL DEFAULT 'none',
  ADD COLUMN IF NOT EXISTS serial_auto_generate boolean NOT NULL DEFAULT true,
  ...
  ADD COLUMN IF NOT EXISTS pos_available boolean NOT NULL DEFAULT true,
  ADD COLUMN IF NOT EXISTS ecommerce_available boolean NOT NULL DEFAULT false,
  ...
  ADD COLUMN IF NOT EXISTS sales_uom varchar(30) NOT NULL DEFAULT 'unit',
  ADD COLUMN IF NOT EXISTS purchase_uom varchar(30) NOT NULL DEFAULT 'unit',
  ADD COLUMN IF NOT EXISTS minimum_sales_price numeric(14,2),
  ADD COLUMN IF NOT EXISTS inventory_account_id uuid,
  ADD COLUMN IF NOT EXISTS cost_of_sales_account_id uuid,
  ADD COLUMN IF NOT EXISTS revenue_account_id uuid,
  ADD COLUMN IF NOT EXISTS purchase_account_id uuid,
  ADD COLUMN IF NOT EXISTS sales_return_account_id uuid,
  ADD COLUMN IF NOT EXISTS purchase_return_account_id uuid,
  ADD COLUMN IF NOT EXISTS deferred_revenue_account_id uuid,
  ADD COLUMN IF NOT EXISTS subscription_revenue_account_id uuid;

CREATE INDEX IF NOT EXISTS products_center_tracking_idx ON products(center_id, inventory_tracking);
...
CREATE INDEX IF NOT EXISTS subscription_revenue_schedule_due_idx ON subscription_revenue_schedules(center_id, recognition_date, status);
